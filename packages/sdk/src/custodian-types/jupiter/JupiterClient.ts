...
export class JupiterClient {
  ...
  async createTransaction(
    ...
    try {
      const response = await fetch(
        `${this.jupiterApiurl}/custodian/transaction`,
        {
          method: "POST",
  ...
  async getTransaction(
    ...
    try {
      const response = await fetch(
        `${this.jupiterApiurl}/custodian/transaction/${custodian_transactionId}`,
        {
          headers,
        }
      );
  ...
  async signTypedData_v4(
    ...
    try {
      const response = await fetch(
        `${this.jupiterApiurl}/custodian/signature`,
        {
          method: "POST",
          body: JSON.stringify(payload),
          headers,
  ...
  async signPersonalMessage(
    accountId: string,
    message: string
  ): Promise<IJupiterPersonalSignatureResponse> {
    const headers = this.getHeaders();
    ...
    try {
      const response = await fetch(
        `${this.jupiterApiurl}/custodian/personal-signature`,
        {
          method: "POST",
          body: JSON.stringify(payload),
          headers,

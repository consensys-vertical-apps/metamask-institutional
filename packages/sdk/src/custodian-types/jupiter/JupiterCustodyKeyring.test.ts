...
} from "../../interfaces/ICustodianAccount";
...
describe("CustodyKeyring", () => {
    ...
    it("accepts an array of custodian accounts and self assigns them", () => {
      ...
      expect(custodyKeyring.selectedAddresses).toEqual(
        mockSelectedAddresses.map(
          custodyKeyring.convertExtensionCustodianAccountToSDKCustodianAccount.bind(
            custodyKeyring
          )
        )
      );
  ...
  describe("getSDK", () => {
    it("returns a copy of the SDK from the sdkList if it exists, or creates one if it is not there", () => {
      ...
      expect(mockedMmiSdkFactory).toHaveBeenCalledWith(
        JupiterCustodianApi,
        { jwt: "jwt" },
        0,
        "apiUrl"
  ...
  describe("getCustodianAccounts", () => {
    it("will get addresses, if the arguments specify an ethereum address", () => {
      ...
      expect(mockMMISDK.getEthereumAccountsByAddress).toHaveBeenCalledWith(
        address,
    ...
    it("will not return already existing addresses", async () => {
      ...
      const result = await custodyKeyring.getCustodianAccounts(
        "jwt",
        "apiUrl",
        address
      );
    ...
    it("will search by labels if there is a non-address search text", () => {
      ...
      expect(
        mockMMISDK.getEthereumAccountsByLabelOrAddressName
      ).toHaveBeenCalledWith("search", DEFAULT_MAX_CACHE_AGE);
    });
  ...
  describe("getTransactionNote", () => {
    it("will get a readable category for transaction based on the transaction category", () => {
      ...
      expect(custodyKeyring.getTransactionNote(txMeta)).toEqual(
        "ETH Transfer - initiated on http://"
      );
    });
  ...
  describe("signTransaction", () => {
    ...
    it("will throw if there's a specified chainId and it doesnt match the account", async () => {
      ...
      await expect(
        custodyKeyring.signTransaction(fromAddress, ethTx, txMeta)
      ).rejects.toThrowError("This network 777 is not supported by Jupiter");
    });
      ...
      expect(mockMMISDK.createTransaction).toHaveBeenCalledWith(
        {
          ...
          maxPriorityFeePerGas: (txMeta.txParams as IEIP1559TxParams)
  ...
  describe("getTransaction", () => {
    it("should call getTransaction on the SDK", async () => {
      ...
      expect(mockMMISDK.getTransaction).toHaveBeenCalledWith(
        "0x123456",
        "12345"
      );
    });
  ...
  describe("getTransactionDeepLink", () => {
    it("should return the URL of the Jupiter Custody UI", async () => {
      ...
      const result = await custodyKeyring.getTransactionDeepLink(
        "0x123456",
        "12345"
      );
  ...
  describe("signMessage", () => {
    it("should throw an exception", () => {
      expect(() => custodyKeyring.signMessage()).toThrowError(
        "Not supported on this custodian"
      );
    });
  });
  ...
  describe("exportAccount", () => {
    it("should throw an exception", () => {
      expect(() => custodyKeyring.exportAccount()).toThrowError(
        "Not supported on this custodian"
      );
    });
  });
  ...
  describe("signTypedData", () => {
    it("should call signTypedData on the SDK", async () => {
      ...
      expect(mockMMISDK.signedTypedData_v4).toHaveBeenCalledWith(
        fromAddress,
        "data",
        "V4"
      );
    ...
    it("throws an exception if an invalid version is specified", () => {
      expect(
        custodyKeyring.signTypedData("0x1236", "data", { version: "V9000" })
  ...
  describe("signPersonalMessage", () => {
    it("should call signPersonalMessage on the SDK", async () => {
      ...
      expect(mockMMISDK.signPersonalMessage).toHaveBeenCalledWith(
        fromAddress,
        "data"
      );
    });

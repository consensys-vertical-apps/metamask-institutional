...
export class QredoClient extends EventEmitter {
  ...
  async getHeaders(): Promise<any["headers"]> {
    const accessToken = await this.cache.tryCachingArray(
      "accessToken",
      QREDO_CACHE_AGE,
      async () => {
  ...
  async getTransaction(
    ...
    try {
      const response = await fetch(
        `${this.apiUrl}/connect/transaction/${custodian_transactionId}`,
        {
          headers,
        }
      );
  ...
  ): Promise<IQredoSignatureResponse> {
    const headers = await this.getHeaders();

    try {
      const response = await fetch(
        `${this.apiUrl}/connect/sign/${custodian_signedMessageId}`,
        {
          headers,
        }
      );
  ...
  async getCustomerProof(): Promise<IQredoCustomerProof> {
    ...
    try {
      ...
      if (e.response?.status >= 400 && e.response?.status < 500) {
        ...
        console.warn(
          "Authentication error obtaining customer proof - deleting token"
        );

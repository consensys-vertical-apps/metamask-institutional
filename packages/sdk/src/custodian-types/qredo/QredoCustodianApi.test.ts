...
describe("QredoCustodianApi", () => {
  ...
  beforeEach(() => {
    qredoCustodianApi = new QredoCustodianApi(
    ...
    mockedQredoClientInstance.getEthereumAccounts = jest
      .fn()
      .mockImplementation(() => qredoAccountsMock);

    mockedQredoClientInstance.getTransactions = jest
      .fn()
      .mockImplementation(() => [qredoTransactionMock]);

    mockedQredoClientInstance.getTransaction = jest
      .fn()
      .mockImplementation(() => qredoTransactionMock);

    mockedQredoClientInstance.createTransaction = jest
      .fn()
      .mockImplementation(() => qredoTransactionMock);

    mockedQredoClientInstance.getCustomerProof = jest
      .fn()
      .mockImplementation(() => qredoCustomerProofMock);

    mockedQredoClientInstance.getSignedMessage = jest
      .fn()
      .mockImplementation(() => qredoSignedMessageMock);

    mockedQredoClientInstance.getNetworks = jest
      .fn()
      .mockImplementation(() => qredoNetworksMock);

    mockedQredoClientInstance.signPersonalMessage = jest
      .fn()
      .mockImplementation(() => qredoPersonalSignResponseMock);

    mockedQredoClientInstance.signTypedData_v4 = jest
      .fn()
      .mockImplementation(() => qredoPersonalSignResponseMock);
    ...
    it("returns the accounts", async () => {
      ...
      expect(result).toEqual(
        qredoAccountsMock.map((account) => ({
          ...
          chainId: qredoCustodianApi.networkMappings.getMappingByCustodianName(
            account.network
          ).chainId,
  ...
  describe("QredoCustodianApi#getEthereumAccountsByAddress", () => {
    it("fetches the ethereum accounts for a given address", async () => {
      const result = await qredoCustodianApi.getEthereumAccountsByAddress(
        "0xb"
      );

      expect(result.length).toBe(
        qredoAccountsMock.filter((acc) => acc.address.startsWith("0xb")).length
      );
    });
  });
  ...
  describe("QredoCustodianApi#getEthereumAccountsByLabelOrAddressName", () => {
    it("fetches the ethereum accounts based on label or address name", async () => {
      const result = await qredoCustodianApi.getEthereumAccountsByLabelOrAddressName(
        "Amazing"
      );

      expect(result.length).toBe(
        qredoAccountsMock.filter((acc) =>
          acc.labels[1].value.includes("Amazing")
        ).length
      );
    });
  });
  ...
  describe("QredoCustodianApi#getSignedMessage", () => {
    it("gets a single SignedMessage by id", async () => {
      const result: ISignatureDetails = await qredoCustodianApi.getSignedMessage(
        "0x",
        qredoSignedMessageMock.sigID
      );
      ...
      expect(mockedQredoClientInstance.getSignedMessage).toHaveBeenCalledWith(
        qredoSignedMessageMock.sigID
      );
    });
  });
  ...
  describe("QredoCustodianApi#getTransaction", () => {
    it("gets a single transaction by id", async () => {
      const result: ITransactionDetails = await qredoCustodianApi.getTransaction(
        "0x",
        qredoTransactionMock.txID
      );
      ...
      expect(mockedQredoClientInstance.getTransaction).toHaveBeenCalledWith(
        qredoTransactionMock.txID
      );
    });
    ...
    describe("QredoCustodianApi#signPersonalMessage", () => {
      it("throws an error", async () => {
        ...
        const result = await qredoCustodianApi.signPersonalMessage(
          fromAddress,
          buffer
        );
        ...
        expect(
          mockedQredoClientInstance.signPersonalMessage
        ).toHaveBeenCalledWith(qredoAccountsMock[0].address, buffer);
      });
    });
    ...
    describe("QredoCustodianApi#signTypedData_v4", () => {
      it("calls client.signTypedData_v4", async () => {
        ...
        const result = await qredoCustodianApi.signTypedData_v4(
          fromAddress,
          buffer,
          "V4"
        );
        ...
        expect(mockedQredoClientInstance.signTypedData_v4).toHaveBeenCalledWith(
          fromAddress,
          buffer
        );
      });

import { IExtensionCustodianAccount } from "../../interfaces/ICustodianAccount";
...
describe("JsonRpcCustodyKeyring", () => {
  ...
  describe("replaceRefreshTokenAuthDetails", () => {
    it("given an address and new auth Details, it calls the sdk with the new auth details", async () => {
      ...
      await custodyKeyring.replaceRefreshTokenAuthDetails(
        "0x123456",
        newAuthDetails
      );
  ...
  describe("updateAccountsDetailsWithNewRefreshToken", () => {
    it("replace the refresh in accountsDetails", () => {
      ...
      custodyKeyring.updateAccountsDetailsWithNewRefreshToken(
        "jwt",
        "newToken",
        "https://api"
      );
      ...
      expect(
        (custodyKeyring.accountsDetails[0]
          .authDetails as IRefreshTokenAuthDetails).refreshToken
      ).toEqual("newToken");
    });
  });
  ...
  describe("getSignature", () => {
    it("should call the sdk with the correct params", async () => {
      ...
      expect(mockMMISDK.getSignature).toHaveBeenCalledWith(
        "0x123456",
        "signed-message-id"
      );
    });

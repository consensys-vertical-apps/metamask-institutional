...
describe("BitgoClient", () => {
  ...
  describe("BitgoClient#getEthereumAccounts", () => {
    ...
    it("should fail if an exception is thrown by the HTTP client", async () => {
      ...
      expect(bitgoClient.getEthereumAccounts()).rejects.toThrow(
        CustodianApiError
      );
    });
  });
  ...
  describe("BitgoClient#createTransaction", () => {
    it("should POST the /custodian/transaction endpoint", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        {
          body:
            '{"txParams":{"to":"test","value":"test","data":"test","gasLimit":"test","gasPrice":"test","from":"test","type":"0"}}',
          headers: {
    ...
    it("should POST the /custodian/transaction endpoint with EIP-1559 params", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        {
          body:
            '{"txParams":{"to":"test","value":"test","data":"test","gasLimit":"test","maxPriorityFeePerGas":"test","maxFeePerGas":"test","from":"test","type":"2"}}',
          headers: {
            Authorization: "Bearer mock-jwt",
            "Content-Type": "application/json",
          },
  ...
  describe("BitgoClient#getTransaction", () => {
    ...
    it("should fail if an exception is thrown by the HTTP client", async () => {
      ...
      expect(bitgoClient.getTransaction("xxx")).rejects.toThrow(
        CustodianApiError
      );
    });
  });

...
export class CurvCustodyKeyring extends CustodyKeyring {
  ...
  handleLegacyAccountDetails(
    detailsArray:
      | ICustodianAccount<ITokenAuthDetails>[]
      | ILegacyCustodianAccount[]
  ): ICustodianAccount[] {
    return detailsArray.map(
      (
        details: ICustodianAccount<ITokenAuthDetails> | ILegacyCustodianAccount
      ) => {
        ...
        if ((details as ILegacyCustodianAccount).jwt) {
          ...
          authDetails = (details as ICustodianAccount<ITokenAuthDetails>)
            .authDetails;
        }
  ...
  deserialize(
    ...
    return super.deserialize({
      ...
      selectedAddresses: opts.selectedAddresses
        ? this.handleLegacyAccountDetails(
            opts.selectedAddresses as ILegacyCustodianAccount[]
          )
        : [],
      accountsDetails: opts.accountsDetails
        ? this.handleLegacyAccountDetails(
            opts.accountsDetails as ILegacyCustodianAccount[]
          )
        : [],
      meta: opts.meta || {},
    });
